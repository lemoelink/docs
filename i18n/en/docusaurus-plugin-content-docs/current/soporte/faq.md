...
title: FAQ
sidebar_position: 2
description: Frequently asked questions about LEMoE.
---
...
### Is LEMoE free?
Yes, LEMoE is open source. Costs come from the backends you configure (e.g., OpenAI API) or the hardware for local models.

### Do I need a GPU to use LEMoE?
No. LEMoE's ML router runs on CPU. The backends you use (Ollama, vLLM) may or may not need a GPU depending on the models you choose.

### Does LEMoE save my conversations?
No. LEMoE is stateless middleware. Prompts pass through and a sanitized summary is logged in `logs/app.log`, but there is no conversation storage.
...
Currently no. Experts are loaded on startup. You must restart LEMoE for changes in `experts.json` to take effect.
...
Yes. LEMoE exposes an OpenAI-compatible API, so LiteLLM can point to it as a provider.
...
### Can I expose LEMoE to the internet?
Not directly. Put it behind a reverse proxy (Nginx/Caddy) with authentication. LEMoE is designed for internal networks and homelabs.

### Does LEMoE filter content?
